...
int main(int argc, char **argv) 
{
	...
	char cmd[256];
	sprintf(cmd,"%s","iostat -x 1 -k > ./log/iostat_spmv.log&");
	std::cout<<cmd<<"\n";
	//exit(-1);
...
#pragma omp barrier
			...
			if(!tid) {
				...
				<<it->cd->io_submit_num<<" "<<it->cd->io_getevents_time<<"\n";
			
				comp_total_tm += comp_tm_end;
				// io_submit_time: load chunk time    io_poll_time: get chunk time
